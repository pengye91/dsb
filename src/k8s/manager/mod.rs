...
impl SandboxManager for KubernetesManager {
    ...
    async fn start(&self, id: &str) -> ManagerResult<()> {
        ...
        // Fetch CRD spec to determine if a custom command was provided.
        // Sandboxes with custom commands don't run the default entrypoint (supervisord),
        // so tool_proxy won't be available. Skip the health check in that case.
        let api: Api<Sandbox> = Api::namespaced(self.client.clone(), &self.namespace);
        ...
        if !has_custom_command {
            ...
            info!(sandbox_id = %id, "Skipping tool_proxy health check: custom command provided");
        }
