...
impl SandboxService {
    ...
    pub(super) async fn wait_for_tool_health(
        ...
        loop {
            ...
            {
                Ok(response) => {
                    // Only check status == "healthy"; the /health endpoint in tool_proxy.py
                    // returns {"status": "healthy", "service": "tool_proxy"} and does not
                    // include a browser_connected field. This is consistent with the K8s
                    // health check in src/k8s/manager/builders.rs.
                    let status = response.get("status").and_then(|v| v.as_str());

                    if status == Some("healthy") {
                        tracing::info!(
                            container_id = %container_id,
                            elapsed_secs = start_time.elapsed().as_secs(),
                            "Tool_proxy is healthy"
                        );
                        return Ok(());
                    } else {
                        tracing::debug!(
