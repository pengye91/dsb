...
|---------|-----|-------------|
| **Dashboard** | <http://localhost:3001> | Configured in `dsb.yaml` or `.env.docker` |
| **API** | <http://localhost:8080> | Configured in `dsb.yaml` or `.env.docker` |
| **Health Check** | <http://localhost:8080/health> | None (public endpoint) |
| **SSH Gateway** | `ssh://localhost:2223` | Authenticated via your SSH keys |

*Note: Default ports can be customized via `.env` / `.env.local`.*
...
| File | Purpose |
|------|---------|
| `.env.example` | Template for infrastructure docker-compose configuration |
| `.env.local` | Your custom infrastructure configuration overrides (optional) |
| `docker/docker-compose.yml` | Service orchestration (utilizes environment variables) |
| `Dockerfile` | Multi-stage DSB core API server container build |
| `dashboard/Dockerfile` | DSB Web Dashboard container build |
| `dsb.yaml.example` | Template for application configuration (Logic limits, API keys, PostgreSQL) |
...
```bash
# Access the live PostgreSQL shell
docker exec -it dsb-postgres psql -U postgres dsb

# Backup the dsb database to a local file
