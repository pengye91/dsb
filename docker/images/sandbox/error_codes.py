...
ErrorCode enum in `src/api/errors.rs`. These codes are used across:
...
2. Add to this file
3. Add to Python SDK error_codes.py
4. Run scripts/verify_error_codes.py to verify sync

# HTTP Status Code Mapping:
- 400: Validation errors, tool validation errors
- 401: Authentication errors
- 403: Authorization errors
- 404: Not found errors (sandbox, tool, container, session)
- 408: Timeout errors (request, tool)
- 409: Conflict errors (invalid state, already exists)
- 429: Rate limit exceeded
- 500: Internal server errors
- 502: Bad gateway (Backend, SSH connection, upstream)
- 503: Service unavailable (database, service)
"""
