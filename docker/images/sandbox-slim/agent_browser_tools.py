...
async def browser_navigate(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not url.startswith(('http://', 'https://', 'data:', 'file://')):
        raise SandboxError('Invalid URL: must start with http://, https://, file://, or data:')
...
async def browser_fill(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ref:
        await run_agent_browser("fill", [ref, value])
    elif selector:
        await run_agent_browser("fill", [selector, value])
    else:
        raise SandboxError("Must provide ref or selector")
...
async def web_scrape(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Open URL in new tab for VNC visibility
