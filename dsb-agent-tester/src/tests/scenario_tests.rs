...
async fn test_web_scraping_pipeline() -> anyhow::Result<()> {
    ...
    // Wait for sandbox to be fully running before scraping
    wait_for_sandbox_running(&agent, &sandbox_id)
        .await
...
async fn test_code_execution_workflow() -> anyhow::Result<()> {
    ...
    // Wait for sandbox to be fully running before executing code
    wait_for_sandbox_running(&agent, &sandbox_id)
        .await
        .context("sandbox readiness wait failed")?;
