...
/// Waits for a sandbox to be fully ready by verifying both container state
/// and tool_proxy accessibility.
///
...
/// state, then verifies that the internal `tool_proxy` is accepting connections
/// by attempting a simple tool call.
pub async fn wait_for_sandbox_running(
    ...
    info!(
        "Waiting for sandbox {} to be fully ready (timeout: {}s)",
        sandbox_id, SANDBOX_READINESS_TIMEOUT_SECS
    );
